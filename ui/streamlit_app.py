...
if st.session_state.results:
    ...
    if plan:
        ...
        with plan_col2:
            ...
            for step in plan.reasoning_steps:
                ...
                if phase == 'THINK':
                    st.markdown(
                        f"<div class='react-step react-think'>"
                        f"<b>💭 THINK:</b> {step.get('observation', '')}<br>"
                        f"<span style='color:#a5b4fc;'>→ {step.get('thought', '')}</span>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                elif phase == 'ACT':
                    st.markdown(
                        f"<div class='react-step react-act'>"
                        f"<b>⚡ ACT:</b> <code>{step.get('action', '')}</code><br>"
                        f"<span style='color:#6ee7b7;'>→ {step.get('detail', '')}</span>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                elif phase == 'OBSERVE':
                    st.markdown(
                        f"<div class='react-step react-observe'>"
                        f"<b>👁️ OBSERVE:</b> {step.get('result', '')}"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                elif phase == 'LLM_REASONING':
                    st.markdown(
                        f"<div class='react-step react-llm'>"
                        f"<b>🤖 LLM Validation:</b> {step.get('output', '')[:400]}"
                        f"</div>",
                        unsafe_allow_html=True
                    )
    ...
    if debate_data:
        ...
        st.markdown(
            "*Three specialized agents (Skill Analyst, Experience Evaluator, Culture Fit Assessor) "
            "debated each candidate. A Devil's Advocate challenged their conclusions, "
            "and the Moderator synthesized the final consensus score.*"
        )

        for di, d in enumerate(debate_data):
            with st.expander(
                f"🏛️ {d.get('candidate', 'Unknown')} — Consensus: {d.get('consensus_score', '?')}/100"
                f" | Skill: {d.get('skill_score', '?')} | Exp: {d.get('experience_score', '?')}"
                f" | Culture: {d.get('culture_score', '?')}"
                f"{'  ⚡' + str(d.get('penalty', 0)) + 'pt penalty' if d.get('penalty', 0) > 0 else ''}",
                expanded=(di == 0)
            ):
                ...
                with dcol1:
                    st.markdown(f"""<div class='metric-card'>
                        <div class='metric-value' style='color: #06b6d4;'>{d.get('skill_score', 0)}</div>
                        <div class='metric-label'>🔧 Skill</div>
                    </div>""", unsafe_allow_html=True)
                with dcol2:
                    st.markdown(f"""<div class='metric-card'>
                        <div class='metric-value' style='color: #8b5cf6;'>{d.get('experience_score', 0)}</div>
                        <div class='metric-label'>💼 Experience</div>
                    </div>""", unsafe_allow_html=True)
                with dcol3:
                    st.markdown(f"""<div class='metric-card'>
                        <div class='metric-value' style='color: #f59e0b;'>{d.get('culture_score', 0)}</div>
                ...
                with dcol4:
                    ...
                    st.markdown(f"""<div class='metric-card'>
                        <div class='metric-value' style='color: #10b981;'>{d.get('consensus_score', 0)}</div>
                        <div class='metric-label'>🏆 Consensus{penalty_text}</div>
                    </div>""", unsafe_allow_html=True)
                ...
                for ev in evaluations:
                    ...
                    st.markdown(
                        f"<div class='debate-agent {css_class}'>"
                        f"<div class='debate-agent-header'>"
                        f"<span class='debate-agent-name'>{icon} {agent} ({ev.get('model', '')})</span>"
                        f"<span class='debate-score-pill'>{ev.get('score', 0)}/100</span>"
                        f"</div>"
                        f"<div style='font-size:0.83rem; color:#475569; white-space: pre-wrap;'>{ev.get('argument', '')[:500]}</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                ...
                if challenges:
                    ...
                    for ch in challenges:
                        ...
                        challenge_text += (
                            ...
                            f"{ch.get('challenge', '')}<br>"
                        )

                    st.markdown(
                        f"<div class='debate-agent devil'>"
                        f"<div class='debate-agent-header'>"
                        f"<span class='debate-agent-name'>⚡ Devil's Advocate</span>"
                        f"<span class='debate-score-pill' style='background:#fee2e2; color:#991b1b;'>"
                        f"{devils.get('challenge_count', 0)} challenges</span>"
                        f"</div>"
                        f"<div style='font-size:0.83rem; color:#475569;'>{challenge_text}</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                ...
                if mod_summary:
                    st.markdown(
                        f"<div class='debate-agent moderator'>"
                        f"<div class='debate-agent-header'>"
                        f"<span class='debate-agent-name'>🏆 Moderator Verdict</span>"
                        f"<span class='debate-score-pill' style='background:#d1fae5; color:#065f46;'>"
                        f"Consensus: {d.get('consensus_score', 0)}/100</span>"
                        f"</div>"
                        f"<div style='font-size:0.83rem; color:#475569; white-space: pre-wrap;'>{mod_summary[:600]}</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
    ...
    st.markdown(
        "*Generate a polished, professional hiring report with visualizations, "
        "comparisons, and actionable recommendations.*"
    )
    ...
    with gen_col1:
        if st.button("🧠 Generate Full Report", key="gen_report", use_container_width=True):
            with st.spinner("Generating report sections..."):
                ...
                st.session_state.report_data = report_agent.generate_full_report(
                    res, job_description=st.session_state.get('last_jd', '')
                )
    ...
    if report:
        ...
        with gen_col2:
            try:
                ...
                st.download_button(
                    "📄 Download PDF",
                    data=pdf_bytes,
                    file_name=f"CognifyX_Report_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
        ...
        with gen_col3:
            try:
                ...
                st.download_button(
                    "📝 Download DOCX",
                    data=docx_bytes,
                    file_name=f"CognifyX_Report_{datetime.now().strftime('%Y%m%d_%H%M')}.docx",
        ...
        if matrix_rows:
            with st.expander("📊 Candidate Comparison Matrix"):
                ...
                display_cols = ['rank', 'name', 'domain', 'experience_years',
                                'ats_score', 'keyword_score', 'llm_score',
                    ...
                    if coverage:
                        for c in coverage:
                            ...
                            if status == 'Strong':
                                ...
                                icon = '🔴'
                            st.markdown(
                                f"{icon} **{c['keyword']}** — {c['coverage_pct']}% coverage "
                                f"({c['candidates_with']} candidates)"
                            )
                with gap_col2:
            ...
            with st.expander("💬 Interview Question Bank"):
                ...
                for cand in iq.get('candidates', []):
                    ...
                    for q in cand['questions']:
                        st.markdown(
                            f"**[{q['type']}]** {q['question']}  \n"
                            f"<small style='color:#64748b;'>📌 {q['reason']}</small>",
                            unsafe_allow_html=True
                        )
                    st.markdown("---")
